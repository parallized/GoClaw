...
function RunPreferencesFields({ form, onChange }: { form: RunPlanRequest; onChange: (next: RunPlanRequest) => void }) {
  ...
  return (
    <fieldset className="border-none p-0 m-0">
      <legend className="mb-6 text-sm font-bold uppercase tracking-widest text-accent-green flex items-center gap-2">
...
function PhotoPreferencesFields({ form, onChange }: { form: PhotoWeekRequest; onChange: (next: PhotoWeekRequest) => void }) {
  ...
  return (
    <fieldset className="border-none p-0 m-0">
      <legend className="mb-6 text-sm font-bold uppercase tracking-widest text-accent-pink flex items-center gap-2">
        <span className="w-1.5 h-1.5 rounded-full bg-accent-pink shadow-[0_0_8px_rgba(173,26,114,0.6)]"></span> 拍照偏好
      </legend>
      <div className="grid gap-x-4 gap-y-6 grid-cols-2">
        <label className="flex flex-col gap-2 overflow-hidden">
          <span className="text-sm font-bold text-secondary uppercase tracking-wider whitespace-nowrap truncate">活动半径(km)</span>
        ...
        <label className="flex flex-col gap-2 overflow-hidden">
          <span className="text-sm font-bold text-secondary uppercase tracking-wider whitespace-nowrap truncate">摄影水平</span>
        ...
        <div className="flex flex-wrap gap-2.5">
          {photoThemes.map((t) => (
            <button
              key={t.value}
              type="button"
              className={`n-toggle px-4 py-2 text-sm font-bold transition-all backdrop-blur-md rounded-full ${prefs?.themes?.includes(t.value) ? "bg-accent-pink/20 text-accent-pink border-accent-pink/50 shadow-[0_0_12px_rgba(173,26,114,0.3)]" : "bg-surface/20 text-secondary border-white/10 hover:bg-surface/40 hover:text-primary"}`}
...
export function FormSection({ scenarioId, runForm, photoForm, onRunChange, onPhotoChange, geoStatus, onRelocate }: FormSectionProps) {
  ...
  return (
    <div>
      ...
      <p className="text-tertiary mb-10 text-sm font-bold uppercase tracking-widest">位置自动获取，偏好自主掌握</p>
